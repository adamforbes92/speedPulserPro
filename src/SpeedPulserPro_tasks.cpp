...
#if serialDebugEEP
    Serial.println("[TASK] EEP: Settings written to EEPROM");
#endif
...
#if serialDebugWifi
    Serial.println("[TASK] UI: Labels updated");
#endif
...
void taskParseGPS(void *parameter)
{
  ...
  while (1)
  {
    ...
    {
      Serial.printf("[TASK] GPS: Speed updated to %d km/h\n", (int)gpsSpeed);
    }
#endif
...
void taskParseDSG(void *parameter)
{
  ...
  while (1)
  {
    ...
    if (dsgSpeed > 0)
    {
      Serial.printf("[TASK] DSG: Speed calculated to %u km/h, Gear: %d\n", dsgSpeed, gear);
    }
#endif
...
void taskProcessRPM(void *parameter)
{
  ...
  while (1)
  {
    ...
    if (vehicleRPM > 0)
    {
      Serial.printf("[TASK] RPM: vehicleRPM=%d, frequencyRPM=%d\n", vehicleRPM, frequencyRPM);
    }
#endif
